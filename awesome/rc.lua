...
awful.util.spawn_with_shell("command -v compton && compton -cC -O 0.1 -I 0.1 -D 3 -f -o 0.1");
...
for s = 1, screen.count() do
  --mystatusbar[s] = statusbar({position = "bottom", height = "18" , name="mystatusbar"})
...
awful.key({ modkey, "Shift" }, "Print", function () os.execute("screenshot area") end),
awful.key({ modkey }, "Print", function () os.execute("screenshot scr") end)
)
